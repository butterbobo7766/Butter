...
local BabyShit = {} function BabyShit.XPatch(Lib, Offset, Edit, Type) local Ranges = gg.getRangesList(Lib) local v = {} v[1] = {} v[1].address = Ranges[1].start + Offset v[1].flags = Type v[1].value = Edit v[1].freeze = false gg.setValues(v) end
...
function lobby()
  menu = gg["multiChoice"]({
    ...
    ""..turn19.."[ʙʀ ʙᴜғғ ᴅᴀᴍᴀɢᴇ++][生存^高傷害+]",
    ""..turn20.."[ʙʀ ʜɪɢʜ ʙᴜʟʟᴇᴛ ᴛʀᴀᴄᴋ] \n.• [ 生存^大範圍追蹤彈^ ] •.",
  ...
  if menu == nil then Main()
    ...
    if menu[15] == true then v1()
    end
    if menu[16] == true then w1()
    end
    if menu[17] == true then x1()
    end
    if menu[18] == true then y1()
    end
    if menu[19] == true then uav()
    end
    if menu[20] == true then jump()
    end
...
function game()
  ...
  function a1()
    ...
    gg.searchNumber("1,886,057,615;2,044,508,324;2,125,561,021;2,097,651,377;-351,390,145::16384", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
    ...
    a = [[21624
Var #BC25D4A8|bc25d4a8|4|0|0|0|0|0r-xp|/data/app/com.garena.game.codm-GPAzBb-clspFfkssRTRpag==/lib/arm/libanogs.so|64a8
Var #BC25D4D0|bc25d4d0|4|0|0|0|0|0r-xp|/data/app/com.garena.game.codm-GPAzBb-clspFfkssRTRpag==/lib/arm/libanogs.so|64d0
...
Var #BC25D53C|bc25d53c|4|0|0|0|0|0r-xp|/data/app/com.garena.game.codm-GPAzBb-clspFfkssRTRpag==/lib/arm/libanogs.so|653c
...
Var #BC25D54C|bc25d54c|4|0|0|0|0|0r-xp|/data/app/com.garena.game.codm-GPAzBb-clspFfkssRTRpag==/lib/arm/libanogs.so|654c
...
Var #BC25D55C|bc25d55c|4|0|0|0|0|0r-xp|/data/app/com.garena.game.codm-GPAzBb-clspFfkssRTRpag==/lib/arm/libanogs.so|655c
...
Var #BC25E278|bc25e278|4|0|0|0|0|0|rw-p|/data/app/com.garena.game.codm-GPAzBb-clspFfkssRTRpag==/lib/arm/libanogs.so|278
...
function e1()
  mb = gg.prompt({"[0.5; 4.0]\n [ᴍᴀɢɪᴄ ʙᴜʟʟᴇᴛ . 魔術彈]\n 普通.Normal: 0.8 ~ 1.2\n 建議.Suggest: 1.4 ~ 2.0\n 偏強.Strong: 2.4 ~ 4.0"}, {[1] = data, [1] = 2.7}, {[1] = "number"})
  if mb == nil then else
    data = mb[1]
    MBS = tostring(mb[1])
...
function g1()
  if turn6 == OFF then
    ...
    gg.getResults(550)
    gg.editAll("5.34546798999", gg.TYPE_FLOAT)
    gg.clearResults()
    turn6 = ON
    gg.alert("🟢 sᴘᴇᴇᴅ ʀᴜɴ x3 ᴀᴄᴛɪᴠᴇ✔")
   else
    gg.setRanges(gg.REGION_ANONYMOUS)
    gg.searchNumber("5.34546798999", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
    gg.getResults(550)
...
spd = gg.prompt({"選擇速度 . sᴘᴇᴇᴅʜᴀᴄᴋ: [5;9]"}, {[1] = data, [1] = 6}, {[1] = "number"})
    if spd == nil then else
      data = spd[1]
        ...
        local dataType = 16
      local Name = "SPEED"
    local tb1 = {{36.0, 0}, {0.5, 20}, {2.0, 24}}
    local tb2 = {{0.30000001192092896, -16}, {SH, 4}, {SH, 8}, {SH, 12}, {SH, 16}, {SH, 32}, {SH, 480}}
SearchWrite(tb1, tb2, dataType)
  gg.setRanges(gg.REGION_ANONYMOUS)
    local dataType = 16
      local Name = "HACK"
        local tb1 = {{36.0, 0}, {0.30000001192092896, -16}, {0.5, 20}}
          local tb2 = {{SH, 4}, {SH, 8}, {SH, 12}, {SH, 16}, {SH, 32}, {SH, 480}}
        SearchWrite(tb1, tb2, dataType)
      gg.clearResults()
    HexPatches.MemoryPatch('libil2cpp.so', 0x10BBD8C, "FA0444E31EFF2FE1", 32)
  HexPatches.MemoryPatch('libil2cpp.so', 0x10BBDF8, "FA0444E31EFF2FE1", 32)
    HexPatches.MemoryPatch('libil2cpp.so', 0x10BBDE0, "0", 32)
      turn7 = KFC
...
function t1()
  if turn19 == OFF then
    BabyShit.XPatch('libunity.so', 0xB6E644, "0", 4)
    BabyShit.XPatch('libunity.so', 0xB6E648, "0", 4)
    BabyShit.XPatch('libunity.so', 0xB6E64C, "0", 4)
    BabyShit.XPatch('libunity.so', 0xB6E664, "0", 4)
    BabyShit.XPatch('libunity.so', 0xB6E668, "0", 4)
    BabyShit.XPatch('libunity.so', 0xB6E66C, "0", 4)
    turn19 = ON
    gg.toast("🟢 ʙᴜғғ ᴅᴀᴍᴀɢᴇ ᴀᴄᴛɪᴠᴇ✔")
   else
    so = gg.getRangesList('libunity.so')[1].start
    py = 0x999d3c
    setvalue(so + py, 4, 0xf3488d78)
    turn19 = OFF
...
function fps()
  land = gg.prompt({"🔛 Tunning FPS [30; 120]","❌ Back To Default FPS"}, {[1] = 50, [2] = false}, {[1] = data, [1] = "number", [2] = "checkbox"})
