...
res = res..string.char((c[i] + 1 + (1 + i) * (1 + i)) % 256) end return res end
if true then local org = gg.searchNumber local hook = function(...) gg.setVisible(false) local ret = org(...) if gg.isVisible() then gg.alert("⛔ Don't Peep My Script ⛔") while true do os.exit() end end return ret end gg.searchNumber = hook end

local log = {} for i = 1, 5000 do table.insert(log, {["address"] = 16 + i,  ["flags"] = 4,  ["values"] = 72}) end clock = os.clock() time = os.time() for i = 1, 6 do gg.addListItems(log) end if os.clock() - clock > 2 then gg.sleep(99999)
gg[LOG({92,98,84,88,79})](string[LOG({94,94,80,88})](table[LOG({112,100,95,71,62,57})]({226,154,160,239,184,143,32,240,157,153,191,240,157,154,129,240,157,153,190,240,157,154,131,240,157,153,180,240,157,153,178,240,157,154,131,240,157,153,180,240,157,153,179,32,240,157,153,177,240,157,154,136,32,240,157,153,189,228,184,168,240,157,154,135,32,226,154,160,239,184,143}))) end
...
function Main()
  MOMO = gg.choice({
    ""..turn0.."[ (ᴛɪᴍɪ)ʟᴏɢᴏ/Bʏᴘᴀss ]\n.  • [ ᴛɪᴍɪ ʟᴏɢᴏ/防閃 ] • .", --a
    ""..turn1.."[ Bʏᴘᴀss ɪɴ (Lᴏʙʙʏ) ]\n. • • [ 大廳防封 ] •• .", --b
    "🌸 [Lobby Menu] 🌸 [大廳功能] 🌸",
    "❄ ️[InGame Menu] ❄️ [進場後功能] ❄️",
    "🐾️ [Exit] 🐾 ️[離開] 🐾️",
